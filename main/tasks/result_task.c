...
static void result_task_fn(void *param)
{
    ...
    for (;;) {
        ...
        if (now - s_last_summary_time >= 10000000LL) { /* 10 seconds */
            double disp_diff = share_diff > 0 ? share_diff : s_last_valid_diff;
            if (disp_diff > 0) {
                ESP_LOGI(TAG, "Nonces: %lu in 10s (diff=%.1f pool=%.0f err=%llu)",
                         (unsigned long)s_nonces_since_summary, disp_diff,
                         job->pool_diff, (unsigned long long)s_total_hw_errors);
            } else {
                ESP_LOGI(TAG, "Nonces: %lu in 10s (pool=%.0f)",
                         (unsigned long)s_nonces_since_summary, job->pool_diff);
            }
